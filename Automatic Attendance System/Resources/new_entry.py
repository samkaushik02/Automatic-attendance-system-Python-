...
def main():
    ...
    ################# ASK FOR DETAILS  ##############
    lbl=Label(fl,font=('Times',30,'bold'),text=" NEW ENTRY ",bg="yellow",fg="#00008B",anchor='w')
    ...
    lbl_id= Label(fl, font=('arial', 15),text=str(id),bd=16,fg="#00008B",anchor="w")
    ...
    def take():
        ...
        for i in range(0,150):
            ...
            if(t==20):
                c=conn.cursor()
                c.execute('INSERT INTO Student_Data  VALUES (?,?,?,?,?,?,?)',(id,roll,Fn,Ln,Sm,Br,Eml))
                conn.commit()
    ...
    #################### BUTTONS  #####################
    btnNw=Button(fl,padx=50,pady=20,bd=15,fg="black",font=('arial',10,'bold'),width=12,text=" SUBMIT \n AND \n TAKE PHOTO ",bg="#00C957",command=take).grid(row=8,column=1,padx=30,pady=20)
